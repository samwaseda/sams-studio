...
class Atom{
	...
	public:
		double *m;
		float  *x;
		Atom() : n_max(0), n_neigh(0), A(0), B(0), E_uptodate(false), acc(0), count(0), type("Fe"), N_spread(4), m_0(0)
		{
			x = new float[3];
			m = new double[3];
			m_old = new double[3];
...
class Coeff{
	private:
		...
		int shell_max;
	public:
		Coeff(string input_file, fstream &ausgabe) : shell_max(6)
		{
			initialize_coeff();
		...
		int shell(){ return shell_max; }

		int Elmax(){return int(pow(2,ID_type.size()))-1;}

		int Defmax(){return int(pow(2,defect_type.size()))-1;}
...
class Shell{
	private:
		...
		string bravais;
	public:
		Shell(int shell_max_in, string bravais) : shell_max(shell_max_in+1){
...
class Energy{
	private:
		...
		class average_energy
		{
			...
			public:
				average_energy(double kappa_in=1.0-1.0e-3): kappa(kappa_in){ reset();}
	...
	public:
		Energy(string input_file, string bravais, int N_in, float N_Mn_in, int N_s, int N_v, double lambda_in, int num_neighbors, bool debug_mode_in): acc(0), MC_count(0), kB(8.6173305e-5), N_rep(N_in), lambda(lambda_in)
		{
